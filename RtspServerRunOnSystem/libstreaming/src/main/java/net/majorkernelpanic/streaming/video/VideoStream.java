...
public abstract class VideoStream extends MediaStream {
	...
	public static boolean onActivity = false;
	/**
	 * Don't use this class directly.
...
//					stopPreview();
					...
					if (isStreaming()) {
						try {
							mSurfaceTexture = new SurfaceTexture(-12313);
...
//							mCamera.setPreviewTexture(mSurfaceView.getSurfaceTexture());
						...
						Log.d(TAG, "surface create new");
					}
					else {
						Log.d(TAG, "surface create old");
					}
				}
				@Override
...
//		mMediaCodec = MediaCodec.createByCodecName(debugger.getEncoderName());
		...
		if(!codecName.equals("OMX.MTK.VIDEO.ENCODER.AVC")){
			convertor.setPlanar(false);
		}
		else{
			convertor.setPlanar(true);
		}
...
//								inputBuffers[bufferIndex].put(data);
	...
	protected void encodeWithMediaCodecMethod2() throws RuntimeException, IOException {
		...
		EncoderDebugger debugger = EncoderDebugger.debug(mSettings, mQuality.resX, mQuality.resY);
		// Tạo encoder
		// name is OMX.google.h264.encoder is using sorfware to encode
		mMediaCodec = MediaCodec.createByCodecName(debugger.getEncoderName());
		// using hardware to encode
		mMediaCodec = MediaCodec.createEncoderByType("video/avc");
		MediaFormat mediaFormat = MediaFormat.createVideoFormat("video/avc", mQuality.resX, mQuality.resY);
		mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, mQuality.bitrate);
		mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, mQuality.framerate);
