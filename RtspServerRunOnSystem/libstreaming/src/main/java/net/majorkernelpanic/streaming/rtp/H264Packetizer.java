...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	private void send() throws IOException, InterruptedException {
		...
		if (naluLength<=MAXPACKETSIZE-rtphl-2) {
			...
			if(type == 7 || type == 8){
				int ppsPos = 0;
				for(int i = rtphl+1; i < len+rtphl; i++){
					if (buffer[i]==0 && buffer[i+1]==0 && buffer[i+2]==0) {
						ppsPos = i+4;
					}
				}
				sps[0] = header[4];
				byte[]newSps = new byte[ppsPos-4-rtphl];
				newSps[0] = header[4];
				byte[]newPps = new byte[len-ppsPos + rtphl +1];
				System.arraycopy(buffer, rtphl+1, newSps,1,ppsPos - 5 - rtphl);
				System.arraycopy(buffer, ppsPos,newPps,0, len+rtphl+1 - ppsPos);
				setStreamParameters(newPps,newSps);
			}
			socket.updateTimestamp(ts);
			socket.markNextPacket();
